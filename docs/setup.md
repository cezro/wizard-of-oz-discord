...
- **Missing DSM nudge:** At the configured nudge time (defaults to summary time if not set), the bot mentions members who have the configured reporter role but have not posted in the standup channel within the rolling 24-hour window. Requires **Server Members Intent** in the Discord Developer Portal.
